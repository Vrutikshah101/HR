...
| ASM-007 | User request “Manager-Engineering, Manager-HW, and so on” is implemented as: add `Hardware` department, add manager designations for Engineering and HW, and map `Assistant Manager`, `Executive`, `General Manager` to all departments. | Assumption | Scope interpretation may differ from organization-specific title matrix. | If needed, refine designations/mappings in master setup screen. |
